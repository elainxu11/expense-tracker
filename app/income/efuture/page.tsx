...
export default async function EfutureDetailPage() {
  const allIncome = await getIncome();
  const entries = allIncome
    .filter((e) => e.source === 'Efuture')
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        <h1 className="text-4xl font-bold text-slate-900">eFuture / YYK</h1>
        <p className="text-slate-500 mt-1">{entries.length} transfer{entries.length !== 1 ? 's' : ''} all time</p>
      </div>

      <div className="grid grid-cols-3 gap-6">
        <div className="bg-gradient-to-br from-violet-50 to-violet-100 rounded-xl p-6 border-2 border-violet-200 shadow">
          <p className="text-sm font-semibold text-violet-700 mb-1">Total Received</p>
          <p className="text-4xl font-bold text-violet-900">${total.toLocaleString('en-US', { minimumFractionDigits: 2 })}</p>
        </div>
        <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl p-6 border-2 border-purple-200 shadow">
          <p className="text-sm font-semibold text-purple-700 mb-1">Transfers</p>
          <p className="text-4xl font-bold text-purple-900">{entries.length}</p>
        </div>
        <div className="bg-gradient-to-br from-slate-50 to-slate-100 rounded-xl p-6 border-2 border-slate-200 shadow">
          <p className="text-sm font-semibold text-slate-600 mb-1">Avg per Transfer</p>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-4">
            {yearTotals.map(({ year, total: yt }) => (
              <div key={year} className="bg-white rounded-xl border-2 border-violet-100 px-6 py-4 shadow-sm min-w-[140px]">
                <p className="text-sm font-semibold text-slate-500">{year}</p>
                <p className="text-2xl font-bold text-violet-900">${yt.toLocaleString('en-US', { minimumFractionDigits: 2 })}</p>
              </div>
            ))}
          </div>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100">
                  ...
                  <th className="text-right px-5 py-3 font-semibold text-slate-600">Transfers</th>
                ...
                {months.map((mk) => {
                  ...
                  return (
                    <tr key={mk} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-5 py-3 text-right font-bold text-violet-900">
      ...
      {entries.length > 0 && (
        <div className="bg-white rounded-xl border-2 border-violet-100 shadow-sm overflow-hidden">
          <div className="px-5 py-4 bg-slate-50 border-b border-slate-200">
            <h2 className="text-lg font-bold text-slate-800">All Transfers</h2>
          </div>
      ...
      {entries.length === 0 && (
        <div className="bg-white rounded-xl border-2 border-slate-200 p-12 text-center">
          <p className="text-slate-400 font-medium text-lg">No eFuture income data found.</p>
        </div>
      )}
    </div>
  );
